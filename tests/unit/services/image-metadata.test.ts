...
describe("ImageMetadataService", () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await fs.mkdtemp(path.join(process.cwd(), "tests", "temp", "image-meta-"));
  });
  ...
  async function createMockJPEG(
    ...
    const sof0Data = Buffer.concat([
      ...
      Buffer.from([(options.height ?? 100) >> 8, (options.height ?? 100) & 0xff]), // Height
  ...
  function createEXIFData(options: any): Buffer {
    ...
    let nextOffset = 8 + 2 + (0 * 12) + 4; // IFD start + count + entries + next IFD pointer

    // Add entries based on options
    if (options.width) {
      entries.push(createIFDEntry(0x0100, 3, 1, options.width)); // ImageWidth
    }
    if (options.height) {
      entries.push(createIFDEntry(0x0101, 3, 1, options.height)); // ImageLength
    }
    if (options.cameraMake) {
      const makeBuffer = Buffer.from(options.cameraMake + "\x00");
      entries.push(createIFDEntry(0x010f, 2, makeBuffer.length, nextOffset));
      nextOffset += makeBuffer.length;
    }
    if (options.cameraModel) {
      const modelBuffer = Buffer.from(options.cameraModel + "\x00");
      entries.push(createIFDEntry(0x0110, 2, modelBuffer.length, nextOffset));
      nextOffset += modelBuffer.length;
    }
    ...
    return Buffer.concat([tiffHeader, ifdCount, ...entries, nextIFD]);
  }

  function createIFDEntry(tag: number, type: number, count: number, value: number | Buffer): Buffer {
  ...
  describe("extract", () => {
    ...
    it("should detect orientation", async () => {
      const filePath = await createMockJPEG("oriented.jpg", {});

      const metadata = await service.extract(filePath);

      expect(metadata.orientation).toBeDefined();
    });
  ...
  describe("GPS Extraction", () => {
    it("should extract GPS coordinates when available", async () => {
      const filePath = await createMockJPEG("gps.jpg", {
        gpsData: { lat: 40.7128, lng: -74.0060 },
      });
      ...
      expect(metadata.gps?.longitude).toBeCloseTo(-74.0060, 2);
    });
  ...
  describe("Privacy Features", () => {
    it("should strip GPS data when requested", async () => {
      ...
      const strippedMetadata = await service.extract(outputPath, { extractGPS: true });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle TIFF images", async () => {
      ...
      const tiffData = Buffer.concat([
        ...
        Buffer.from([0x00, 0x01, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00]), // Width = 100
  ...
  describe("Integration Tests", () => {
    it("should handle complete workflow with privacy stripping", async () => {
      ...
      const originalMeta = await service.extract(originalPath, { extractGPS: true });
      ...
      const strippedMeta = await service.extract(strippedPath, { extractGPS: true });
    ...
    it("should organize batch with mixed metadata", async () => {
      ...
      files.push(await createMockJPEG("full.jpg", {
        cameraMake: "Canon",
        cameraModel: "EOS",
        width: 1920,
        height: 1080,
      ...
      // Image with GPS
      files.push(await createMockJPEG("gps.jpg", { gpsData: { lat: 40, lng: -74 } }));
