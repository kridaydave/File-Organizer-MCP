...
describe("DuplicateFinderService", () => {
  ...
  afterEach(async () => {
    try {
      ...
      // Use mock logger instead of console.error
  ...
  describe("findWithScoring", () => {
    ...
    it("should score files based on strategy: best_location", async () => {
      ...
      const docScore =
        group.files.find((f) => f.path === docFile.path)?.score || 0;
      const dlScore =
        group.files.find((f) => f.path === dlFile.path)?.score || 0;
  ...
  describe("deleteFiles", () => {
    ...
    it("should delete files with different content if user requests (trust user intent)", async () => {
      ...
      const result = await duplicateFinder.deleteFiles(
        [dup1.path, dup2.path, unique.path],
        { autoVerify: false },
      );
    ...
    it("should handle single file deletion (edge case)", async () => {
      ...
      const result = await duplicateFinder.deleteFiles([file.path], {
        autoVerify: false,
      });
    ...
    it("should reject deleting last copy of file (auto-verify enabled)", async () => {
      const uniqueFile = await createFile(
        "unique_verify.txt",
        "unique content",
      );

      const result = await duplicateFinder.deleteFiles([uniqueFile.path], {
        autoVerify: true,
    ...
    it("should allow deleting when duplicate exists (auto-verify enabled)", async () => {
      ...
      const result = await duplicateFinder.deleteFiles([file2.path], {
        autoVerify: true,
      }); // autoVerify=true
