  ...
  try {
    ...
    await test('Page 1 (limit=2, offset=0)', async () => {
      ...
      if (data.returned_count !== 2)
        throw new Error(`Expected returned_count=2, got ${data.returned_count}`);
      if (data.total_count !== 5)
        throw new Error(`Expected total_count=5, got ${data.total_count}`);
      if (data.has_more !== true) throw new Error('Expected has_more=true');
      if (data.next_offset !== 2) throw new Error('Expected next_offset=2');
      if (data.items[0]?.name !== 'file1.txt')
        throw new Error(`Expected file1.txt, got ${data.items[0]?.name}`);
    });
    ...
    await test('Page 2 (limit=2, offset=2)', async () => {
      ...
      if (data.items.length !== 2) throw new Error(`Expected 2 items, got ${data.items.length}`);
      if (data.items[0]?.name !== 'file3.txt')
        throw new Error(`Expected file3.txt, got ${data.items[0]?.name}`);
      if (data.has_more !== true) throw new Error('Expected has_more=true');
    ...
    await test('Page 3 (limit=2, offset=4)', async () => {
      ...
      if (data.items[0]?.name !== 'file5.txt')
        throw new Error(`Expected file5.txt, got ${data.items[0]?.name}`);
