...
async function checkAccess(
    ...
    try {
        ...
        if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
            ...
            return { accessible: false, reason: 'No accessible parent directory found' };
        }
        return { accessible: false, reason: (error as Error).message };
    }
}
...
export async function validatePathBase(
    ...
    const { realPath } = await resolveSymlinks(absolutePath);
    ...
    if (requireExists || checkWrite) {
        ...
        if (!accessible) {
            throw new AccessDeniedError(rawValidatedPath, reason);
        }
    }
...
export class PathValidatorService {
    ...
    async validatePath(inputPath: unknown, options: Omit<ValidatePathOptions, 'basePath' | 'allowedPaths'> = {}): Promise<string> {
        return validatePathBase(inputPath, {
            ...options,
