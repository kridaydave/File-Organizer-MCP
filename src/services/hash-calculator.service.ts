...
export class HashCalculatorService {
    ...
    constructor(maxFileSize = 100 * 1024 * 1024) { // 100MB default
    ...
    async calculateHash(fileInput: string | fs.FileHandle): Promise<string> {
        ...
        try {
            if (typeof fileInput === 'string') {
                ...
                stream = fileInput.createReadStream({ start: 0, highWaterMark: 64 * 1024, autoClose: false });
            }
     ...
     */
    async findDuplicates(files: FileWithSize[], options: { timeoutMs?: number } = {}): Promise<DuplicateGroup[]> {
        ...
        for (const file of files) {
            ...
            if (Date.now() - startTime > timeoutMs) {
                throw new Error(`Duplicate analysis timed out after ${timeoutMs}ms. Processed ${Object.keys(hashMap).length} files.`);
            }
