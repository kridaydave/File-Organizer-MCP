...
export const WatchDirectoryInputSchema = z
    .object({
        ...
        schedule: z
            ...
            .describe('Cron expression (e.g., "0 9 * * *" for 9am daily, "*/30 * * * *" for every 30 min)'),
        auto_organize: z
            .boolean()
            .default(true)
        ...
        min_file_age_minutes: z
            ...
            .min(0)
            .optional()
            .describe('Minimum file age in minutes before organizing (prevents organizing files being written)'),
        max_files_per_run: z
            .number()
            .int()
            .min(1)
...
export const watchDirectoryToolDefinition: ToolDefinition = {
    ...
    inputSchema: {
        ...
        properties: {
            directory: { 
                type: 'string', 
                description: 'Full path to the directory to watch (e.g., "C:\\Users\\John\\Desktop\\Work-Notes")' 
            },
            schedule: { 
                type: 'string', 
                description: 'Cron expression. Convert natural language to cron: "every day at 10am" → "0 10 * * *", "every 30 minutes" → "*/30 * * * *", "every Monday at 9am" → "0 9 * * 1", "every hour" → "0 * * * *", "daily at midnight" → "0 0 * * *"' 
            },
            auto_organize: { 
                type: 'boolean', 
