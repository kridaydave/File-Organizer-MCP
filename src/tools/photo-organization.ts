...
export const OrganizePhotosInputSchema = z
  .object({
    ...
    target_dir: z
      ...
      .describe('Full path to the directory where organized photos will be placed'),
    date_format: z
...
export const organizePhotosToolDefinition: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      target_dir: { 
        type: 'string', 
        description: 'Full path to the directory where organized photos will be placed' 
      },
      date_format: { 
        type: 'string', 
        enum: ['YYYY/MM/DD', 'YYYY-MM-DD', 'YYYY/MM', 'YYYY'],
...
export async function handleOrganizePhotos(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
