...
export async function handleGetCategories(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
    ...
    const response_format = parsed.success ? parsed.data.response_format : 'markdown';
...
export async function handleSetCustomRules(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
    ...
    // Apply to singleton
    const appliedCount = globalCategorizerService.setCustomRules(rules as CustomRule[]);

    if (appliedCount === 0) {
      return {
        content: [{ type: 'text', text: 'No valid Custom Rules were applied.' }],
      };
    }
