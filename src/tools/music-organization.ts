...
export const OrganizeMusicInputSchema = z
  .object({
    ...
    target_dir: z
      ...
      .describe('Full path to the directory where organized music will be placed'),
    structure: z
...
export const organizeMusicToolDefinition: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      target_dir: { 
        type: 'string', 
        description: 'Full path to the directory where organized music will be placed' 
      },
      structure: { 
        type: 'string', 
        enum: ['artist/album', 'album', 'genre/artist', 'flat'],
...
export async function handleOrganizeMusic(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
