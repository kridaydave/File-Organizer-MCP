...
export const BatchReadFilesInputSchema = z
  .object({
    ...
    max_file_size_mb: z
      ...
      .optional()
      .default(10)
      .describe('Maximum file size in MB to read content (larger files get metadata only)'),
    include_content: z
      .boolean()
      .optional()
      .default(true)
    ...
    file_types: z
      .array(z.string())
      .optional()
      .describe('Filter by specific file extensions (e.g., [".txt", ".pdf"]). Empty = all files'),
  })
...
export const batchReadFilesToolDefinition: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      max_files: { type: 'number', description: 'Maximum files to process (safety limit)', default: 50 },
      max_file_size_mb: { type: 'number', description: 'Max file size in MB to read content', default: 10 },
      include_content: { type: 'boolean', description: 'Include text content', default: true },
...
const TEXT_EXTENSIONS = new Set([
  ...
  '.swift', '.kt', '.scala', '.r', '.pl', '.sh', '.bash', '.zsh', '.fish',
  '.html', '.htm', '.css', '.scss', '.sass', '.less', '.vue', '.svelte',
  '.sql', '.graphql', '.gql',
  '.log', '.ini', '.conf', '.config', '.properties', '.env',
  '.pdf', '.docx', '.doc', // These need special handling but contain text
]);
...
const MEDIA_EXTENSIONS = new Set([
  ...
  '.raw', '.cr2', '.cr3', '.nef', '.arw', '.dng', '.orf', '.rw2', '.pef', '.sr2', '.raf',
]);

function determineContentType(ext: string): 'text' | 'media' | 'binary' | 'unknown' {
...
async function readTextFile(filePath: string, maxSizeBytes: number): Promise<string | undefined> {
  try {
    const stats = await fs.stat(filePath);
    if (stats.size > maxSizeBytes) {
      return `[File too large (${(stats.size / 1024 / 1024).toFixed(2)} MB) - reading first ${(maxSizeBytes / 1024).toFixed(0)} KB only]`;
    }

    const ext = path.extname(filePath).toLowerCase();
    
    // For now, simple text reading. Could be enhanced with PDF parsers, etc.
    if (ext === '.pdf' || ext === '.docx' || ext === '.doc') {
      return `[${ext} file - content extraction not implemented, use metadata for context]`;
    }

    const content = await fs.readFile(filePath, 'utf-8');
    
    // Truncate very long content
    const maxLength = 50000; // 50KB of text max
...
export async function handleBatchReadFiles(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
    ...
    if (file_types && file_types.length > 0) {
      ...
      files = allFiles.filter(f => allowedExts.has(path.extname(f.name).toLowerCase()));
    }

    // Limit max files for safety
    if (files.length > max_files) {
      logger.warn(`Batch read limited to ${max_files} files (found ${files.length})`);
    ...
    for (const file of files) {
      ...
      const result: FileReadResult = {
        ...
        extension: ext,
        size: file.size,
        category: contentType === 'media' ? 'Media' : contentType === 'text' ? 'Text' : 'Other',
        contentType,
      };
      ...
      try {
        ...
        if (include_metadata) {
          if (contentType === 'media') {
            ...
            const lowerExt = ext.toLowerCase();
            if (['.mp3', '.flac', '.ogg', '.wav', '.m4a', '.aac', '.wma', '.opus'].includes(lowerExt)) {
              try {
                ...
                result.metadata = {
                  ...
                  duration: audioMeta.duration ? `${Math.round(audioMeta.duration / 60)}:${String(Math.round(audioMeta.duration % 60)).padStart(2, '0')}` : undefined,
                ...
                result.metadata = { type: 'audio', error: 'Failed to extract audio metadata' };
              }
            } else if (['.jpg', '.jpeg', '.png', '.tiff', '.tif', '.gif', '.bmp', '.webp', '.heic', '.heif'].includes(lowerExt)) {
              try {
                ...
                result.metadata = {
                  ...
                  camera: imageMeta.cameraMake || imageMeta.cameraModel ? `${imageMeta.cameraMake || ''} ${imageMeta.cameraModel || ''}`.trim() : undefined,
                  hasGPS: imageMeta.hasGPS,
                  hasEXIF: imageMeta.hasEXIF,
                };
              } catch (e) {
                result.metadata = { type: 'image', error: 'Failed to extract image metadata' };
              }
            } else if (['.mp4', '.avi', '.mkv', '.mov', '.wmv', '.flv', '.webm', '.m4v'].includes(lowerExt)) {
              // Video metadata - use basic file stats for now
              result.metadata = { type: 'video', note: 'Video metadata extraction not implemented' };
            }
          } else {
    ...
    if (response_format === 'json') {
      return {
        content: [{ type: 'text', text: JSON.stringify({ directory: validatedPath, fileCount: results.length, files: results }, null, 2) }],
        structuredContent: { directory: validatedPath, fileCount: results.length, files: results } as Record<string, unknown>,
      };
    }
    ...
    for (const [cat, items] of Object.entries(grouped)) {
      const desc = cat === 'Media' ? 'Audio/Video/Images (metadata only)' : cat === 'Text' ? 'Text files (content + metadata)' : 'Binary/Unknown files';
    ...
    for (const [category, items] of Object.entries(grouped)) {
      ...
      for (const item of items) {
        ...
        if (item.content) {
          ...
          const displayContent = item.content.length > 500 
