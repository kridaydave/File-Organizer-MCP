...
export const AnalyzeDuplicatesInputSchema = z
    .object({
        ...
        recommendation_strategy: z.enum(['newest', 'oldest', 'best_location', 'best_name']).default('best_location'),
...
export const analyzeDuplicatesToolDefinition: ToolDefinition = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            recommendation_strategy: { type: 'string', enum: ['newest', 'oldest', 'best_location', 'best_name'], default: 'best_location' },
...
export async function handleAnalyzeDuplicates(
    ...
    try {
        ...
        const analyzed = await duplicateFinder.findWithScoring(files, recommendation_strategy);

        const summary = {
            total_duplicate_groups: analyzed.length,
            total_duplicate_files: analyzed.reduce((sum, g) => sum + g.file_count - 1, 0),
            total_wasted_space_bytes: analyzed.reduce((sum, g) => sum + g.wasted_space_bytes, 0),
            total_wasted_space_readable: formatBytes(analyzed.reduce((sum, g) => sum + g.wasted_space_bytes, 0))
        };

        if (response_format === 'json') {
            return {
                content: [{ type: 'text', text: JSON.stringify({ summary, duplicate_groups: analyzed }, null, 2) }],
...
**Delete:**
${g.files.slice(1).map(f => `- \`${f.path}\` (Score: ${f.score})
  - ${f.reasons.join(', ')}`).join('\n')}
`).join('\n')}
`;
...
export async function handleDeleteDuplicates(
    ...
    try {
        ...
        const { files_to_delete, create_backup_manifest, response_format } = parsed.data;
        const duplicateFinder = new DuplicateFinderService();

        const result = await duplicateFinder.deleteFiles(files_to_delete, { createBackupManifest: create_backup_manifest });
