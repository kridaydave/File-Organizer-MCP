...
export async function handleAnalyzeDuplicates(
  ...
  try {
    ...
    if (response_format === 'json') {
      return {
        content: [
          { type: 'text', text: JSON.stringify({ summary, duplicate_groups: analyzed }, null, 2) },
        ],
...
export async function handleDeleteDuplicates(args: Record<string, unknown>): Promise<ToolResponse> {
  try {
