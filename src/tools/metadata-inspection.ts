...
export async function handleInspectMetadata(args: Record<string, unknown>): Promise<ToolResponse> {
    try {
        ...
        if (category) {
            try {
                const extractedMetadata = await metadataService.extractMetadata(validatedPath, fileExtension);
                ...
                result.warnings?.push(`Could not extract metadata: ${metadataError instanceof Error ? metadataError.message : String(metadataError)}`);
            }
        } else {
        ...
        if (category === 'Images') {
            result.warnings?.push('Note: GPS/Location data is excluded from metadata extraction for privacy protection');
        }
