...
// Statistics
interface TOCTOUStats {
  testsRun: number;
  attacksBlocked: number;
  attacksSucceeded: number;
  errors: number;
  byTest: Map<string, { blocked: number; succeeded: number; errors: number }>;
}

const stats: TOCTOUStats = {
  testsRun: 0,
  attacksBlocked: 0,
  attacksSucceeded: 0,
  errors: 0,
...
function updateStats(
  testName: string,
  result: "blocked" | "succeeded" | "error",
): void {
  stats.testsRun++;
  if (result === "blocked") stats.attacksBlocked++;
  else if (result === "succeeded") stats.attacksSucceeded++;
  else stats.errors++;
...
async function testRapidFileReplacement(
  ...
  console.log(
    `${colors.blue}Test 1: Rapid file replacement attack${colors.reset}`,
  );
...
async function testConcurrentSymlink(
  ...
  console.log(
    `${colors.blue}Test 3: Concurrent symlink creation${colors.reset}`,
  );
...
async function testDirectoryTraversalSymlink(
  ...
  console.log(
    `${colors.blue}Test 4: Directory traversal via symlink${colors.reset}`,
  );
...
async function testONOFollowProtection(
  ...
  console.log(
    `${colors.blue}Test 5: O_NOFOLLOW protection verification${colors.reset}`,
  );
  ...
  console.log(
    `  O_NOFOLLOW blocked: ${onoFollowBlocked}, bypassed: ${onoFollowBypassed}`,
  );
}
...
async function testConcurrentAccessPatterns(
  ...
  console.log(
    `${colors.blue}Test 7: Concurrent access patterns${colors.reset}`,
  );
...
`);
  ...
  console.log(
    `${colors.green}Attacks Blocked: ${stats.attacksBlocked}${colors.reset}`,
  );
  console.log(
    `${colors.red}Attacks Succeeded: ${stats.attacksSucceeded}${colors.reset}`,
  );
  console.log(`Errors: ${stats.errors}`);

  const blockRate =
    stats.testsRun > 0 ? (stats.attacksBlocked / stats.testsRun) * 100 : 0;
  ...
  for (const [testName, testStats] of stats.byTest.entries()) {
    ...
    console.log(
      `  ${testName}: ${color}blocked=${blocked}, succeeded=${succeeded}${colors.reset}`,
    );
  }

  const status =
    stats.attacksSucceeded === 0 ? `${colors.green}PASS` : `${colors.red}FAIL`;
 ...
 */
async function runTOCTOUTests(): Promise<number> {
  console.log(`
${colors.blue}╔════════════════════════════════════════════════════════════════╗
