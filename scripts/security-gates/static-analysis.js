...
const securityRules = [
    // Rule 1: Direct fs.readFile without validation
    {
        ...
        description: "Direct fs.readFile without path validation can lead to path traversal",
        severity: "CRITICAL",
        pattern: /fs\.readFile\s*\(/,
        excludePattern: /pathValidator|validatePath|SecureFileReader|openAndValidateFile/,
    ...
    // Rule 2: fs.readFileSync without validation
    {
        ...
        description: "fs.readFileSync without validation is vulnerable to path traversal",
        severity: "CRITICAL",
    ...
    // Rule 5: Missing audit log in file operations
    {
        ...
        severity: "MEDIUM",
        pattern: /async\s+(read|write|delete|move|copy)\s*\([^)]*\)\s*\{[\s\S]{0,500}?\}/,
    ...
    // Rule 14: Debug mode in production
    {
        ...
        severity: "MEDIUM",
        pattern: /debug\s*:\s*true|DEBUG\s*=\s*true|NODE_ENV\s*=\s*["']development["']/i,
    ...
    // Rule 19: Information disclosure in errors
    {
        ...
        severity: "MEDIUM",
        pattern: /res\.send\s*\(\s*(error|err|e)\s*\)|res\.json\s*\(\s*\{[^}]*(error|stack|trace)/i,
        ...
        for (const rule of securityRules) {
            ...
            for (const match of matches) {
                ...
                if (rule.excludePattern &&
                    rule.excludePattern.test(content.substring(match.index || 0, (match.index || 0) + 500))) {
                    continue;
                }
                ...
                stats.bySeverity.set(rule.severity, (stats.bySeverity.get(rule.severity) || 0) + 1);
            }
        }
    }
    catch (error) {
        stats.filesSkipped++;
        console.error(`${colors.yellow}Warning: Could not read ${filePath}${colors.reset}`);
    }
}
/**
...
`);
    ...
    for (const [ruleId, count] of stats.byRule.entries()) {
        ...
        if (rule) {
            const color = rule.severity === "CRITICAL"
                ? colors.red
                : rule.severity === "HIGH"
                    ? colors.yellow
                    : colors.cyan;
            console.log(`  ${color}[${ruleId}] ${rule.name}: ${count}${colors.reset}`);
        }
    }
    ...
    if (issues.length > 0) {
        ...
        const printIssues = (issueList, color, label) => {
            if (issueList.length > 0) {
                console.log(`${color}=== ${label} (${issueList.length}) ===${colors.reset}`);
    ...
    const status = criticalCount === 0 && highCount === 0
        ? `${colors.green}PASS`
        : `${colors.red}FAIL`;
...
`);
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if ((i + 1) % 10 === 0 || i === files.length - 1) {
            process.stdout.write(`\r${colors.blue}Progress: ${i + 1}/${files.length} files analyzed${colors.reset}`);
        }
    }
