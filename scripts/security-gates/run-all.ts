...
async function runGate(
  ...
  try {
    ...
    console.error(
      `${colors.red}Gate execution failed: ${error}${colors.reset}`,
    );
  }
...
async function generateReport(): Promise<void> {
  ...
  console.log(
    `${colors.cyan}Security report saved to: ${reportPath}${colors.reset}\n`,
  );
}
 ...
 */
function printRequirementsChecklist(): void {
  ...
  for (const req of requirements) {
    console.log(
      `  ${colors.blue}[${req.id}]${colors.reset} ${req.description}`,
    );
 ...
 * Main runner
 */
async function runAllGates(): Promise<number> {
  printBanner();
  printRequirementsChecklist();

  console.log(
    `${colors.yellow}Starting security gate execution...${colors.reset}\n`,
  );
