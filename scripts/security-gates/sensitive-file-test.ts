...
// Statistics
interface SensitiveFileStats {
  totalTests: number;
  blocked: number;
  allowed: number;
  byCategory: Map<string, { blocked: number; allowed: number }>;
}

const stats: SensitiveFileStats = {
  totalTests: 0,
  blocked: 0,
  allowed: 0,
  byCategory: new Map(),
 ...
 */
function getSensitiveFileTestCases(): Array<{
  path: string;
  category: string;
  description: string;
}> {
...
async function setup(): Promise<void> {
  ...
  await fs.writeFile(
    path.join(ALLOWED_DIR, "secrets.json"),
    '{"secret": "data"}',
  );
...
`);
  ...
  console.log(
    `${colors.red}Allowed (Security Risk): ${stats.allowed}${colors.reset}`,
  );

  const blockRate =
    stats.totalTests > 0 ? (stats.blocked / stats.totalTests) * 100 : 0;
  ...
  const status =
    stats.allowed === 0 ? `${colors.green}PASS` : `${colors.red}FAIL`;
 ...
 */
async function runSensitiveFileTests(): Promise<number> {
  console.log(`
${colors.blue}╔════════════════════════════════════════════════════════════════╗
...
`);
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    if ((i + 1) % 50 === 0) {
      process.stdout.write(
        `\r${colors.blue}Progress: ${i + 1}/${testCases.length} tests completed${colors.reset}`,
      );
    }
  }
  ...
  if (failedCases.length > 0) {
    ...
    console.log(
      `${colors.red}These represent potential information disclosure vulnerabilities.${colors.reset}\n`,
    );
