...
function generatePathTraversalPayloads() {
    ...
    basicSequences.forEach((p) => payloads.push({
        payload: p,
        category: "basic",
    ...
    encodedPayloads.forEach((p) => payloads.push({
        payload: p.payload,
        category: "encoding",
    ...
    unicodePayloads.forEach((p) => payloads.push({
        payload: p.payload,
        category: "unicode",
    ...
    casePayloads.forEach((p) => payloads.push({
        payload: p.payload,
    ...
    doubleEncodingPayloads.forEach((p) => payloads.push({
        payload: p.payload,
    ...
    nullBytePayloads.forEach((p) => payloads.push({
        payload: p.payload,
        category: "null-byte",
        description: p.description,
    }));
    ...
    normalizationPayloads.forEach((p) => payloads.push({
        payload: p.payload,
    ...
    windowsPayloads.forEach((p) => payloads.push({
        payload: p.payload,
    ...
    appSpecificPayloads.forEach((p) => payloads.push({
        payload: p.payload,
    ...
    schemePayloads.forEach((p) => payloads.push({
        payload: p.payload,
        category: "scheme",
        description: p.description,
    }));
    ...
    mixedPayloads.forEach((p) => payloads.push({
        payload: p.payload,
        category: "mixed",
        description: p.description,
    }));
 ...
 */
async function testPayload(reader, payload, category) {
    try {
...
`);
    ...
    console.log(`${colors.red}Passed (Security Risk): ${stats.passed}${colors.reset}`);
...
async function setup() {
    try {
        ...
        await fs.writeFile(path.join(ALLOWED_READ_PATH, "safe.txt"), "safe content");
    }
    catch (error) {
...
async function runFuzzing() {
    ...
    if (payloads.length < BLOCKED_COUNT_THRESHOLD) {
        console.log(`${colors.yellow}Warning: Generated fewer than ${BLOCKED_COUNT_THRESHOLD} payloads${colors.reset}`);
    }
    // Test each payload
    const failedPayloads = [];
    for (let i = 0; i < payloads.length; i++) {
        const { payload, category, description } = payloads[i];
        ...
        if ((i + 1) % 100 === 0) {
            process.stdout.write(`\r${colors.blue}Progress: ${i + 1}/${payloads.length} payloads tested${colors.reset}`);
        }
    }
    ...
    if (failedPayloads.length > 0) {
        ...
        console.log(`${colors.red}These represent potential security vulnerabilities.${colors.reset}\n`);
...
//# sourceMappingURL=path-traversal-fuzz.js.map