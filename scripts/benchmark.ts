...
interface BenchmarkReport {
  ...
  summary: {
    ...
    overallThroughputMbPerSec: number;
  };
  ...
  try {
    ...
    const report: BenchmarkReport = {
      ...
      summary: {
        ...
        totalFilesRead,
        totalBytesRead,
        overallThroughputFilesPerSec: Math.round(
          (totalFilesRead / totalDuration) * 1000,
        ),
        overallThroughputMbPerSec: parseFloat(
          (((totalBytesRead / totalDuration) * 1000) / (1024 * 1024)).toFixed(
            2,
          ),
        ),
      },
...
async function benchmarkFileSize(
  ...
  return {
    ...
    throughput: {
      ...
      mbPerSecond: parseFloat(
        (((sizeConfig.bytes / mean) * 1000) / (1024 * 1024)).toFixed(2),
      ),
    },
    memory: {
      initialMb:
      ...
      deltaMb:
        Math.round(
          ((peakMemory - initialMemory.heapUsed) / (1024 * 1024)) * 100,
        ) / 100,
    },
  };
}

function checkThresholds(results: BenchmarkResult[]): boolean {
  for (const result of results) {
    if (result.latencies.p95 > THRESHOLDS.maxP95LatencyMs) {
      return false;
    }
    if (
      result.throughput.filesPerSecond < THRESHOLDS.minThroughputFilesPerSec
    ) {
      return false;
    }
...
function printHumanSummary(report: BenchmarkReport): void {
  ...
  console.error(
    `  Total Files Read: ${report.summary.totalFilesRead.toLocaleString()}`,
  );
  console.error(
    `  Total Bytes Read: ${(report.summary.totalBytesRead / (1024 * 1024)).toFixed(2)} MB`,
  );
  console.error(
    `  Throughput: ${report.summary.overallThroughputFilesPerSec} files/sec`,
  );
  console.error(
    `  Data Rate: ${report.summary.overallThroughputMbPerSec} MB/sec`,
  );
